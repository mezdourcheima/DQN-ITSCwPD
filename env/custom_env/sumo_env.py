...
class SumoEnv:
    ...
    @staticmethod
    def pretty_print(d):   #Formats and prints JSON data.
    ...
    def gen_route_net(self):
        node_ids = [node.getID() for node in self.net.getNodes()]
        non_tl_nodes = [n for n in node_ids if n not in self.tl_ids]
        ...
        for (on, dn) in itertools.permutations(non_tl_nodes, 2):
            ...
            shortest_path = self.net.getShortestPath(outgoing_edges[0], incoming_edges[0])
            if shortest_path is not None and shortest_path[0] is not None:
                route_edges = [edge.getID() for edge in shortest_path[0]]
